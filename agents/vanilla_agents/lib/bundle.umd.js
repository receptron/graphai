(function (global, factory) {
    ...
    const pushAgent = async ({ namedInputs, params, }) => {
        ...
        const arrayCopy = array.map((item) => item); // shallow copy
        if (item !== undefined) {
            arrayCopy.push(item);
        }
        if (items) {
    ...
    const arrayFindFirstExistsAgent = async ({ namedInputs }) => {
        agent_utils.arrayValidate("arrayFindFirstExistsAgent", namedInputs);
        return namedInputs.array.find((item) => !graphai.isNull(item));
    };
    ...
    exports.arrayFindFirstExistsAgent = arrayFindFirstExistsAgentInfo;
