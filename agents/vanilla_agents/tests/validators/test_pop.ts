...
test("test pop_agent validator ok 1", async () => {
  ...
  const params = {};
  const res = await popAgent.agent({ inputs: [], inputSchema: popAgent.inputs, namedInputs, params, filterParams: {}, debugInfo: { verbose: false, nodeId: "test", retry: 5 } });
...
test("test pop_agent validator error", async () => {
  ...
  await assert.rejects(
    async () => {
      await popAgent.agent({ inputs: [], inputSchema: popAgent.inputs, namedInputs, params, filterParams: {}, debugInfo: { verbose: false, nodeId: "test", retry: 5 } });
    },
