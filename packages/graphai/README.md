...
Here is a simple example, which uses Wikipedia as the data source and performs an in-memory RAG (Retrieval-Augmented Generation).
...
- *inputs*: a set of inputs came from other nodes (specified by the "inputs" property of the node).
...
An *inline agent function* is a simplified version of an *agent function*, which is embedded in the graph (available only when the graph was described in TypeScript). An *inline agent function* receives the *inputs* parameter as its only argument.
...
- *anyInput*: An optional boolean flag (default is false), which indicates that the associated *agent function* will be called when at least one of the input data becomes available. Otherwise (default), it will wait until all the data becomes available.
...
- *priority*: An optional number, which specifies the priority of the execution of the associated agent (the task). The default is 0, which means "neutral". Negative numbers are allowed as well.
...
The databaseQuery node (which is associated with "nestedAgent") takes the data from the "question" node and "projectId" node, and makes them available to inner nodes (nodes of the child graph) via the phantom node, ":question" and ":projectId". After the completion of the child graph, the data from the "query" node (which has the "isResult" property) becomes available as a property of the output of the "database" node.
...
This mechanism does not only allow developers to reuse code, but also make it possible to execute the child graph on another machine using a "remote" agent, enabling the *distributed execution* of nested graphs.
...
Here is an example, which performs an LLM query for each person in the list and creates the list of answers. The "people" node (static) is initialized with an array of names, and the "retriever" node (computed) retrieves one name at a time, and sends it to the "query" node (computed) to perform an LLM query. The "reducer" appends it to the array retrieved from the "result" node (static node, which is initialized as an empty array).

The "update" property of two static nodes ("people" and "result"), updates those properties based on the results from the previous iteration. This loop continues until the value of the "people" node becomes an empty array.
...
If the size of the array is N, the mapAgent creates N instances of a GraphAI object and runs them concurrently.
...
Since the task queue is shared between the parent graph and the child graph (unless the graph is running remotely), tasks created by the child graph will be bound by the same concurrency specified by the parent graph.

Since the task executing the nested graph will be in "running" state while tasks within the child graph are running, the concurrency limit will be incremented by one when we start running the child graph and restored when it is completed.
