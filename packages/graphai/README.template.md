...
- *inputs*: a set of inputs that come from other nodes (specified by "inputs" property of the node).
...
An *inline agent function* is a simplified version of *agent function*, which is embedded in the graph (available only when the graph is described in TypeScript). An *inline agent function* receives the *inputs* parameter as its only argument.
...
- *anyInput*: An optional boolean flag (default is false), which indicates that the associated *agent function* will be called when at least one of the input data became available. Otherwise (default), it will wait until all the data became available.
...
The databaseQuery node (which is associated with "nestedAgent") takes the data from "question" node and "projectId" node, and makes them available to inner nodes (nodes of the child graph) via phantom node, ":question" and ":projectId". After the completion of the child graph, the data from "query" node (which has "isResult" property) becomes available as a property of the output of "database" node.
...
This mechanism does not only allow developers to reuse code, but also makes it possible to execute the child graph on another machine using a "remote" agent, enabling the *distributed execution* of nested graphs.
...
If the size of the array is N, the mapAgent creates N instances of GraphAI object, and runs them concurrently.
