...
export const getNestedGraphData = (
  graphData: GraphData | string | undefined,
  inputs: Array<any>,
): GraphData => {
...
export const nestedAgent: AgentFunction<{
  ...
  if (taskManager) {
    ...
    assert(
      ...
      `nestedAgent: Concurrency is too low: ${status.concurrency}`,
    );
  }
  ...
  namedInputs.forEach((nodeId, index) => {
    if (nestedGraphData.nodes[nodeId] === undefined) {
      ...
      (nestedGraphData.nodes[nodeId] as StaticNodeData)["value"] =
        inputs[index];
    }
  });
