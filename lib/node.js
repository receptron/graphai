...
class ComputedNode extends Node {
    ...
    prepareExecute(transactionId, previousResult) {
        const log = (0, log_1.executeLog)(this.nodeId, this.retryCount, transactionId, this.agentId, this.params, previousResult);
        this.graph.appendLog(log);
        this.state = type_1.NodeState.Executing;
        this.transactionId = transactionId;
        return log;
    }
    errorProcess(error, transactionId, log) {
        if (!this.isCurrentTransaction(transactionId)) {
