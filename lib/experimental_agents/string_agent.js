...
exports.stringSplitterAgent = exports.stringTemplateAgent = exports.getStringInput = void 0;
// If the input is string, return it as-is. Otherwise, return its iputKey property.
const getStringInput = (input, inputKey) => {
...
const stringTemplateAgent = async ({ params, inputs }) => {
    ...
    const content = inputs.reduce((template, input, index) => {
        return template.replace("${" + index + "}", (0, exports.getStringInput)(input, inputKey));
    }, params.template);
    return { content };
};
...
const stringSplitterAgent = async ({ params, inputs }) => {
    const source = inputs[0][params.inputKey ?? "content"];
